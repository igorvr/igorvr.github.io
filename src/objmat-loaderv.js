AFRAME.registerComponent('objmat-loaderv', {
  schema: {
    nmap: {type: 'string'},
	envmap: {type:'string'},
	obj: {type: 'model'},
...
//nmapLoader = new THREE.TextureLoader();
	  //nmapLoader.load(nmapUrl, function (materials) {
        objLoader.load(objUrl, function (objModel) {
		  ...
		  if( envmapUrl ){
			objModel.children[0].material.envMap = new THREE.TextureLoader().load(envmapUrl);
			objModel.children[0].material.envMap.minFilter = THREE.LinearFilter;
			objModel.children[0].material.envMap.mapping = THREE.EquirectangularRefractionMapping;
		  }
		  if( reflectivity ){
  ...
  tick: function(){
	  if(this.el.object3D.children[0]){
		    this.el.object3D.children[0].children[0].material.envMap.image = this.vid;
			this.el.object3D.children[0].children[0].material.envMap.needsUpdate=true;
	  }
	}
});	
